function Func = P2011_dynamics_Red(t,y,P,sunrise,sunset)
% 
...
% Edinburgh, licensed under the Apache License, Version 2.0
    ...
    Func(1) = 1* (q1*YHBL*P+n1*g1^a/(g1^a+(PRR9p+PRR7p+PRR5p+TOCp)^a))-LHYm*(m1*YHBL+m2*(1-YHBL)); % LHY mRNA 
    ...
    Func(5) = (p2+p1*YHBL)*LHYm-m3*LHYp-p3*LHYp^c/(LHYp^c+g3^c); % LHY prot
    Func(6) = 1*n2*g4/(g4+EC)*(g5^e/(g5^e+LHYp^e))-TOCm*m5; % TOC1 mRNA
    Func(7) = p8*PRR9m-(m13+m22*(1-YHBL))*PRR9p; % PRR9 prot
    Func(8) = 1*(n10*LHYpm^e/(g12^e+LHYpm^e)+n11*PRR7p^b/(g13^b+PRR7p^b))-m16*PRR5m; % PRR5 (NI) mRNA
    Func(9) = p10*PRR5m-(m17+m24*(1-YHBL))*PRR5p; % PRR5 (NI) prot
    Func(10)= p11*GIm-m19*GIp-p12*L*ZTL*GIp+p13*GIZ*(1-L)-p17*ELF3pc*GIp-p28*GIp+p29*Gn; % GI prot cytoplasm
    Func(11)= p4*TOCm-m8*TOCp-(m6+m7*(1-YHBL))*TOCp*(p5*ZTL+GIZ); % TOC1 prot
    Func(12)= 1*p14-m20*ZTL-p12*L*ZTL*GIp+p13*GIZ*(1-L); % ZTL
    Func(13)= p26*LUXp*e34-m36*EC*COP1pnn-m37*EC*COP1pnd-m32*EC*(1+p24*YHBL*(EGn+Gn)^d/(g7^d+(EGn+Gn)^d)); % EC
    Func(14)= 1*(q2*YHBL*P+g15^e/(g15^e+LHYp^e)*g14/(g14+EC)*n12)-GIm*m18; % GI mRNA
    Func(15)= 1*q3*YHBL*P+g8/(g8+EC)*(n4+n7*LHYp^e/(LHYp^e+g9^e))-m12*PRR9m; % PRR9 mRNA
    Func(16)= 1*(n8*(LHYp+LHYpm)^e/(g10^e+(LHYp+LHYpm)^e)+n9*PRR9p^f/(g11^f+PRR9p^f))-m14*PRR7m; % PRR7 mRNA
    Func(17)= p9*PRR7m-PRR7p*(m15+m23*(1-YHBL)); % PRR7 prot
